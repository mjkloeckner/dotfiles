...
function JumpBuffer()
	...
	if bufexists(l:string)
		...
		if filereadable(l:string)
			...
			exe "e " .. l:string
			mod
			echo "Buffer doesn't exists: editinig new file `".. l:string .. "`"
		endif
	endif
endfunction
...
function FindFile()
	echohl Question
	let l:input = split(input("Find file: ", expand('%'), "file"), ' ')
	echohl None
	if empty(l:input)
		return
	endif
...
nnoremap <C-x><C-f> :call FindFile()<CR>
...
function MarkdownMode()
	...
	autocmd TermOpen * set cmdheight=0 | set laststatus=0 | exe 
				\"au WinClosed <buffer> set cmdheight=1 | set laststatus=1 | mod"
				\| nnoremap <silent> <buffer> <Esc> :close<CR> | mod
	nnoremap <silent> <buffer> <F11> :call GeneratePDF()<CR>
	nnoremap <silent> <buffer> <F12> :call OpenPDF()<CR>
	nnoremap <silent> <tab> :TocToggle<CR>
